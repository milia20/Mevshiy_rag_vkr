...
MODEL_NAME = "qwen/qwen3.6-35b-a3b"  # or any other model you want to use
OUTPUT_CSV = "evaluation_results_qwen.csv"
...
def evaluate_ru_rag(ollama_client: OllamaClient, df: pd.DataFrame, sample_size: int = 50) -> list[dict[str, Any]]:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Evaluating Russian RAG dataset"):
        ...
        is_correct = (
            generated_answer.lower() in correct_answer.lower() or correct_answer.lower() in generated_answer.lower()
        )
...
def evaluate_natural_questions(
    ollama_client: OllamaClient, nq_test_file: str, sample_size: int = 50
) -> list[dict[str, Any]]:
    """Evaluate the model on the Natural Questions test dataset."""
    results = []

    # Load Natural Questions test data
    with open(nq_test_file, encoding="utf-8") as f:
...
def main():
    ...
    if os.path.exists(ru_rag_path):
        ...
        print(f"Evaluating on {min(sample_size, len(df_ru_rag))} samples from Russian RAG dataset...")
    ...
    nq_test_path = os.path.join(
        "../datasets/natural-questions-master", "nq-test-sample.jsonl"
    )  # Update with actual path
    if os.path.exists(nq_test_path):
        print(f"Evaluating on Natural Questions dataset from {nq_test_path}...")
        nq_results = evaluate_natural_questions(ollama_client, nq_test_path, sample_size=sample_size)
    ...
    if all_results:
        ...
        print("\n=== Example Results ===")
        for _, row in df_results.sample(min(3, len(df_results))).iterrows():
