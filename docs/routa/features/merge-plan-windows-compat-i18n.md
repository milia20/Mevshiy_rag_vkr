...
- **CLI 新 crate**：`routa-cli`（fitness/fluency 模块）、`routa-entrix`（evidence/governance/scoring）
...
**本分支对 kanban 的改动**：主要是 i18n 字符串替换 + 少量格式化（`handleSpecialistLanguageChange` →
`_handleSpecialistLanguageChange`），这些在重写后的上游版本中不再相关。
...
- `routa-entrix` 新 crate：evidence/governance/model/scoring（~3300 行）
