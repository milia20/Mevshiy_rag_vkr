...
- `crates/routa-cli/src/commands/chat.rs` explicitly supports `@` at the start of a message to open an interactive
  specialist picker.
...
- `crates/routa-cli/src/main.rs` now exposes `routa agent run` with:
    ...
    - `-p/--prompt`
    - `-d/--specialist-dir`
- `crates/routa-cli/src/commands/agent.rs` resolves specialists from configured directories and also supports
  prompt-level specialist mention parsing.
