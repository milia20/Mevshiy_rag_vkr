...
`/settings/harness` 当前的 `Execution plan` 还是静态 summary cards + dimension 列表。它能展示数量和 runner 分发，但不能解释
`Filter -> Dispatch -> Gates -> Report` 的执行路径，也不能把 dimension / metric / hard gate 之间的关系作为可交互拓扑展开。
