...
The `create_agent` coordination tool currently only supports basic role-based agent creation. Users cannot specify which
**ACP provider** to use (e.g., opencode, claude, gemini) or reference a **specialist ID** for custom agent
configurations.

This limits flexibility in multi-agent workflows where:

- Different agents need to use different providers (e.g., use claude-opus for ROUTA coordinator, opencode for CRAFTER
  implementers)
