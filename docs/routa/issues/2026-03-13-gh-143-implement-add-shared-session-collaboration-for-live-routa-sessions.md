...
Add a shared-session collaboration capability to Routa so a second user can attach to an active coordination/chat
session, observe the live event stream, and optionally participate by sending prompts or comments under host-controlled
permissions.

This should bring the collaboration model of tools like `claude-duet` into Routa, but adapted to Routa's architecture:
shared **agent/orchestration sessions**, not shared terminal control.
...
But it does not appear to support a first-class way for another human collaborator to join an active session and
...
Introduce a **shared session** abstraction that allows multiple human participants to attach to one active Routa session
while preserving a single execution authority and explicit host control.
...
The shared-session layer should subscribe to the host session's update stream and republish those updates to all
attached collaborators.
