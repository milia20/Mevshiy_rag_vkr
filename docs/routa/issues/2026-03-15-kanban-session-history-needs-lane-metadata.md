...
Card detail can show historical session IDs again, but the underlying task data still stores session history as a flat
`sessionIds` array.

That means the UI can show chronological runs, but it cannot always reconstruct the exact lane, specialist, or
transition reason for each historical session in more complex flows such as:
...
As a result, the UI has to infer lane history from the current board order, which works for the happy path but is not
authoritative for non-linear workflows.
