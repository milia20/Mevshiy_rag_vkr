...
Build a TypeScript-based CLI for Routa that acts as an ACP orchestrator — managing multiple ACP agents (ROUTA, CRAFTER,
GATE) to fulfill user requests from the terminal. The primary interface is:
...
The web UI already manages multiple ACP services visually. The CLI should expose the same orchestration power for
terminal-first workflows, CI/CD pipelines, and scripted automation — without requiring a browser or running server.
