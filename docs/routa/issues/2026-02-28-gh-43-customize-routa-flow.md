...
This issue tracks the implementation of a customizable multi-agent workflow system that orchestrates the complete
software development lifecycle from issue creation to deployment verification.
...
> Routa 的核心 Agent 运行时基于 `ClaudeCodeSdkAdapter`（`src/core/acp/claude-code-sdk-adapter.ts`），封装
`@anthropic-ai/claude-agent-sdk`，通过 JSONL stream 与 `cli.js` 通信。
