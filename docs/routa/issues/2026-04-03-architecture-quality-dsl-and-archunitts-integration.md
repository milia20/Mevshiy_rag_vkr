...
当前仓库已经具备 `docs/fitness/*.md -> entrix -> /api/fitness/* -> Harness/Fitness UI` 的完整治理链路，但“后端 core
架构质量”仍缺少独立、结构化、可视化友好的度量层。

目前只有 `.dependency-cruiser.cjs` 提供仓库级、粗粒度的 TypeScript 依赖边界约束；对于 `src/core/**` 这类 TypeScript
backend core，还没有更细粒度的架构规则执行与结果展示。

同时，如果后续希望引入类似 `guarding` 的可视化与多语言能力，直接复用 `ArchUnitTS` 自带 beta HTML report 并不合适，因为展示文案和模板并不适合
Routa 当前的 i18n 体系。
