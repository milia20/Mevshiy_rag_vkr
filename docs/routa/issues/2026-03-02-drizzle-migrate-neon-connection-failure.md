...
3. The error message doesn't guide users toward resolution (e.g., "DATABASE_URL must point to a Neon serverless
   instance")
...
1. Updating `drizzle.config.ts` to use the `postgres` driver (via drizzle-orm) which supports both local and remote
   connections
