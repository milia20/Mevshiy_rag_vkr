...
- 另一端是 `docs/fitness/rust-api-test.md` 中的 endpoint 级 API 回归与
  `crates/routa-server/tests/rust_api_end_to_end.rs` 中的大型端到端测试
...
- Environment: both
- Trigger: 分析 Rust fitness 结构时，发现 `unit-test.md` 与 `rust-api-test.md` 之间缺少一层能够直接承载业务用例的测试对象，导致很多与业务相关的断言只能通过
  handler 级 API 测试间接覆盖
...
- `crates/routa-server/src/application/sessions.rs` 新增 session 应用层，承载 list/history/context 的合并、筛选、排序和
  fallback 规则，并提供独立 Rust 测试。
- `crates/routa-server/src/api/tasks.rs` 与 `crates/routa-server/src/api/sessions.rs` 已改为委托 application service
  处理业务编排。
