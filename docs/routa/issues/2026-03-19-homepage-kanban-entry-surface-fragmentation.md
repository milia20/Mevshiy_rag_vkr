...
- Homepage tests and product copy are drifting because the homepage is simultaneously treated as a landing page,
  launcher, and Kanban summary
...
- Test expectations still encode multiple homepage narratives (`Kanban-First Control Surface`, `Open board`, older
  `Kanban Core` copy)
...
- Decision for now: keep `/workspace/{workspaceId}` as an overview route instead of redirecting, because it still
  provides recovery/context functions that are distinct from active board execution
