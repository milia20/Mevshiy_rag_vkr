...
6. Prefer the highest-ROI, lowest-risk split first: extract only one or two high-coupling modules per pass before
   considering broader decomposition.
...
3. If still tied, prefer higher-change files from `entrix analyze long-file` history signals (for example
   `commitCount`).
