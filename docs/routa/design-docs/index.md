...
The repository still contains historical design material under `.kiro/specs/`. Those files are useful, but they are not
yet normalized into the `docs/` information architecture.
...
- When a legacy spec becomes canonical, create a focused document here and link back to the source in a short provenance
  note.
