...
Finish the documentation migration for the historical workspace-centric redesign and use that migration to drive cleanup
of the remaining transition logic.

## Why This Plan Exists

The repository already implements most of the workspace-first model, but the durable description of that model was
trapped in `.kiro/specs/`. Meanwhile, some runtime paths still behave as if `default` is a normal long-term workspace
instead of a bootstrap artifact.
...
5. Retire or downgrade the old `.kiro/specs/workspace-centric-redesign/*` files once the canonical docs cover the
   necessary architecture and active cleanup paths.
...
- New contributors no longer need to read `.kiro/specs/workspace-centric-redesign/*` first to understand the current
  architecture.
