...
This guide covers the process for releasing new versions of Routa CLI to multiple distribution channels: **crates.io** (
Cargo), **npm**, and **GitHub Releases**.
...
- `CRATE_TOKEN` - Get from [crates.io/me](https://crates.io/me) → API Tokens (Note: The workflow uses `CRATE_TOKEN`, not
...
If a crate version already exists on crates.io, the workflow will skip it and continue. This is normal for patch
re-releases.
...
**Root Cause**: The `sync-release-version.mjs` script doesn't sync Rust crate versions (only Desktop Tauri and npm
packages).
...
**Issue**: `.github/workflows/release.yml` doesn't call `cargo-release.yml`, so Rust crates won't be published
automatically.

**Workaround**: Manually trigger the Cargo release workflow or publish locally as described in "Troubleshooting > Cargo
Publish Fails".
...
**Issue**: Windows runners default to PowerShell, which doesn't handle `${RELEASE_VERSION}` environment variables the
same way as bash, causing version parsing errors.
