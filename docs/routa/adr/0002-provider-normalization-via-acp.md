...
Routa.js orchestrates multiple agent runtimes: Claude Code SDK (stream-json), OpenCode (ACP-native), and potentially
others. Each provider emits different event shapes, manages processes differently, and has distinct installation/warmup
requirements.

The question was whether to let provider-specific protocol details leak through the system (faster to add new providers)
or normalize everything behind a single protocol layer (harder per provider, but simpler domain and UI code).
...
- New providers must implement an adapter that normalizes output to unified session updates. The domain layer never sees
  raw provider events.
