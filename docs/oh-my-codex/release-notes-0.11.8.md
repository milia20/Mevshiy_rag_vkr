...
`0.11.8` is a hotfix release after `0.11.7` that disables all nudges while deep-interview state is active and hardens
duplicate fresh-leader nudge prevention.
...
- The coverage is targeted at notify-hook and fallback-watcher nudge paths; broader runtime behavior still relies on the
  full suite and live tmux workflows.
