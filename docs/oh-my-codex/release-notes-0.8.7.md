...
OMX's instruction surfaces now have a cleaner shared contract across the root orchestrator, templates, generated
guidance, and the prompt catalog.
...
Team orchestration received a hardening pass focused on expired-claim recovery, worktree hygiene, and stronger
regression coverage.
...
OMX's MCP stdio entrypoints now share one idempotent shutdown path instead of duplicating raw transport bootstrap logic
in each server.
...
Analyst, planner, and other fast-path agent defaults were tuned downward to better match the intended routing posture
