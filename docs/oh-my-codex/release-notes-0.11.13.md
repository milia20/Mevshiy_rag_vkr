...
`0.11.13` is a patch release after `0.11.12` that restores the release branch to a buildable state, improves
team/runtime delivery integrity for leader nudges and mailbox handoff, and keeps Windows/worktree supervision paths more
stable.

## Included fixes and changes

- restore `src/hooks/__tests__/notify-fallback-watcher.test.ts` after accidental placeholder corruption on the release
  branch
...
- The patch includes broad runtime/team surfaces since `0.11.12`, so post-release monitoring should pay extra attention
  to leader mailbox/nudge behavior and Windows worktree flows.
