...
- The required packed-install smoke hydration fix landed later on `dev` in
...
The packed-install smoke workflow now copies and resolves hydration assets from the local smoke workspace so release
verification matches packaged-install behavior more reliably.
