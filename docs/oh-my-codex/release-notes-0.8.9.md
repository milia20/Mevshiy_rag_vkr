...
This hotfix release finishes the team worker startup path so routed task roles are not just inferred during planning —
they are carried into the actual worker startup instruction surface and live worker metadata.
...
Dynamic scaling now writes new tasks through canonical team state before worker bootstrap, so scaled workers receive
stable task ids, persisted roles, and inbox/task metadata that matches the runtime contract used by initial team
startup.
