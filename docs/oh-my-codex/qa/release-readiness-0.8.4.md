...
- Release validation uncovered two additional quality issues during final gating: a watcher shutdown cleanup race in one
  streaming test and an unused setup prompt path caught by the strict no-unused check. Both were resolved and the full
  release gates were rerun cleanly.
