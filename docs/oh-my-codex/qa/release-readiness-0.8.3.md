...
- Test-only hardening for `src/hooks/__tests__/notify-fallback-watcher.test.ts` to stabilize full-suite verification
  under load
...
- A secondary validation risk was a flaky watcher test under full-suite load; that test was hardened to wait for watcher
  readiness and the full clean suite now passes.
