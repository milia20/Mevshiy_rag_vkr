...
Commands use `#[serde(tag = "command")]` — the variant name becomes the `"command"` field, remaining fields are
flattened inline:
...
Events use `#[serde(tag = "event")]` — the variant name becomes the `"event"` field, remaining fields are flattened
inline:
