...
The `RuntimeEngine` (in `crates/omx-runtime-core/src/engine.rs`) writes the following files via `persist()` and
...
All files are written atomically to the configured `state_dir`. TS readers must treat these files as read-only; the Rust
