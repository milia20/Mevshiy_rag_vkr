...
This guide covers migration from **v0.4.4** to the current mainline changes merged after it (including PR #137 and
follow-up fixes).
...
- `omx setup` now supports scope-aware install modes (`user`, `project`). Legacy `project-local` values are
  auto-migrated.
...
- [ ] Confirm removed references are gone from local notes/scripts:
  ```bash
  rg -n "deep-executor|scientist|pipeline|project-session-manager|\bpsm\b|ultrapilot|learn-about-omx|writer-memory|learner|deepinit|\brelease\b" README_indexer.md docs scripts .omx -S
  ```
