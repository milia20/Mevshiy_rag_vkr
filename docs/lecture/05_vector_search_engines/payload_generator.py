...
def random_datetime() -> str | datetime:
    ...
    if "z" in fmt:
        random_datetime = random_datetime.replace(tzinfo=timezone(offset=timedelta(hours=random.randint(-12, 12))))
...
def one_random_payload_please(idx: int) -> dict[str, Any]:
    payload = {
        ...
        "nested_array": [
            [random_signed_int() for _ in range(random.randint(0, 5))] for _ in range(random.randint(0, 5))
        ],
